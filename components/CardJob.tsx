...
const CardJob: React.FC<CardJobProps> = ({ image, title, city, date, type, color = "blue", id }) => {
  ...
  {/* DESKTOP */}
    ...
    {/* MOBILE */}
    <a href={`/${id}-${slugify(title)}`} className="grid grid-cols-3 md:hidden  hover:-mt-2 focu:-mt-2  transition-all duration-500 ease-in-out	 border border-gray-200     font-bold  rounded-lg bg-white">
      ...
      <div className="w-full justify-between flex text-sm flex-col col-span-2 p-2">
        <span className="text-primary uppercase">{title}</span>
